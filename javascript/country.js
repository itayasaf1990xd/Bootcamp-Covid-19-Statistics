...
const generateCountryChart = (deaths, confirmed, recovered, critical) => {
  const data = [deaths, confirmed, recovered, critical];
...
const generateBtn = (countries) => {
  ...
  countries.forEach((country) => {
    ...
    buttons += `<li class="buttons n1" onclick="generateCountryChart(${testingArr})" data-name=${country.name}><a>${country.name}</a></li>`;
    testingArr = [];
  });
